...
func (UnimplementedGARServiceServer) TriggerSession(*TriggerSessionRequest, grpc.ServerStreamingServer[TriggerSessionResponse]) error {
	return status.Error(codes.Unimplemented, "method TriggerSession not implemented")
}
...
func RegisterGARServiceServer(s grpc.ServiceRegistrar, srv GARServiceServer) {
	// If the following call panics, it indicates UnimplementedGARServiceServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
